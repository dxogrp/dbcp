...
- Unified direct and slack-penalized solves under `BiconvexProblem.solve()`.
  ...
  options gives effective defaults of `1` and `1e-6`, respectively; explicitly
  supplying either option in direct mode is rejected. The `method` keyword is
  available for CVXPY and passes through to each subproblem solve; replace the
  former DBCP selector `method="penalty"` with `mode="penalty"`. The old
  `slack_tolerance` keyword is rejected with this error:
  `'slack_tolerance' was removed; use 'slack_tol'.`

  Penalty results are classified as feasible when total slack is less than or
  equal to `slack_tol`, including when both values are zero.
