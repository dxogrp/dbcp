...
A penalty-mode status containing `infeasible` also emits a warning with the
final constraint-violation total. Increasing `nu`, improving the initial
values, or using another convex solver can produce a more feasible point.
